...
const STATUSES    = ['active', 'on_trip', 'rest', 'inactive'] as const;
const STATUS_CLS: Record<string, string> = {
  active: 'pill-green', on_trip: 'pill-blue', rest: 'pill-warn', inactive: 'pill-red',
};
...
export default function Drivers() {
  ...
  const createMut = useMutation<Driver, Error, Partial<Driver>>({
    mutationFn: (data) => driversService.create(data),
    onSuccess:  () => { qc.invalidateQueries({ queryKey: ['drivers'] }); toast.success('Driver added'); closeModal(); },
    onError:    () => toast.error('Failed to add driver'),
  });

  const updateMut = useMutation<Driver, Error, { id: string; data: Partial<Driver> }>({
    mutationFn: ({ id, data }) => driversService.update(id, data),
    onSuccess:  () => { qc.invalidateQueries({ queryKey: ['drivers'] }); toast.success('Driver updated'); closeModal(); },
    onError:    () => toast.error('Failed to update'),
  });

  const deleteMut = useMutation<unknown, Error, string>({
    mutationFn: (id) => driversService.remove(id),
    onSuccess:  () => { qc.invalidateQueries({ queryKey: ['drivers'] }); toast.success('Driver deleted'); setConfirmId(null); },
    onError:    () => toast.error('Failed to delete'),
  });

  function openCreate() {
    reset({ name: '', truck_type: 'Flatbed', terminal: '', status: 'active', phone: '' });
    setEditing(null);
    setShowModal(true);
  }
  function openEdit(d: Driver) {
    reset({ name: d.name, truck_type: d.truck_type, terminal: d.terminal, status: d.status, phone: d.phone ?? '' });
    setEditing(d);
    setShowModal(true);
  }
    ...
    <>
      <div className="metric-row">
        <div className="metric-mini">
          <div className="metric-mini-label">Active Drivers</div>
          <div className="metric-mini-value" style={{ color: 'var(--accent)' }}>{metrics?.active ?? '—'}</div>
        </div>
        <div className="metric-mini">
          <div className="metric-mini-label">On Trip</div>
          <div className="metric-mini-value">{metrics?.on_trip ?? '—'}</div>
        </div>
        <div className="metric-mini">
          <div className="metric-mini-label">Avg Loads / Driver</div>
          <div className="metric-mini-value">{metrics?.avg_loads_per_driver?.toFixed(1) ?? '—'}</div>
        </div>
        <div className="metric-mini">
          <div className="metric-mini-label">Total</div>
          <div className="metric-mini-value">{items.length}</div>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="card-header">
          <div>
            <div className="card-title">Drivers Leaderboard</div>
            <div className="card-subtitle">Sorted by revenue</div>
          </div>
          {canEdit && (
            ...
            <tbody>
              {items.map((d: Driver) => (
                <tr key={d.id}>
                  ...
                  <td className="num">{(d as any).loads ?? 0}</td>
                  <td className="num" style={{ color: 'var(--accent)' }}>{fmt((d as any).revenue ?? 0)}</td>
                  <td className="num">{fmtN((d as any).miles ?? 0)}</td>
                  <td className="num" style={{ color: 'var(--warn)' }}>{fmt((d as any).fuel ?? 0)}</td>
                  <td className="num">{(d as any).rpm ?? '—'}</td>
                  {canEdit && (
                    <td>
      ...
      <div className="card">
        ...
        {top8.length > 0 && (
          <DriversBarChart
            labels={top8.map((d: any) => d.name.split(' ')[0])}
            values={top8.map((d: any) => d.revenue ?? 0)}
          />
        )}
      </div>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={createMut.isPending || updateMut.isPending}>
                  {(createMut.isPending || updateMut.isPending) && (
                    <span className="spinner" style={{ width: 12, height: 12, borderTopColor: '#0d0f14' }} />
                  )}
        ...
        <div className="modal-backdrop">
          <div className="modal" style={{ width: 360 }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--muted)' }}>
              This driver will be deactivated. Are you sure?
            </p>
            <div className="modal-footer">
